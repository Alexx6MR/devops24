...
To ensure that the mariadb service is started and running, you can do it in two ways. 

1. Use ansible to connect to mariadb  using the command: `ansible dbserver -m command -a "systemctl status mariadb"`.
**output**
```bash
...
2. Use ssh to connect to dbserver and run a command: `ssh dbserver -t 'systemctl status mariadb; bash -l'`.
**output**
```bash
● mariadb.service - MariaDB 10.11 database server
     Loaded: loaded (/usr/lib/systemd/system/mariadb.service; enabled; preset: disabled)
...
2. With Ansible:
    * `ansible dbserver -m command -a "systemctl status mariadb"` -> returns mysqld is alive.